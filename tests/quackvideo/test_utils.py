...
class TestFrameUtils:
    ...
    def test_scene_change_detection(self, sample_frames):
        ...
        assert detect_scene_change(
            black_frame, white_frame, threshold=0.3, method="mae"
        )
        assert detect_scene_change(
            black_frame, white_frame, threshold=0.3, method="ssim"
        )
    ...
    def test_similar_frames_finding(self, sample_frames):
        ...
        similar = list(
            find_similar_frames(black_frame, iter(frames), threshold=0.1, method="mse")
        )
